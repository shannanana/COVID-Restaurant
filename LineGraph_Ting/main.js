...
d3.text('./data/Seated_Diner_Data.csv').then(function(text) {
	...
	// Load Covid data
	d3.csv('./data/US_Covid_Data.csv').then(function(dataset) {
		d3.csv('./data/US_State_Names.csv').then(function(stateNameMapping) {
			let stateNameDict = {};
...
function updateDinerChart(dateIndexRange=selectedDateIndexRange, region=selectedRegionList[selectedRegion]) {
	...
	let circlesG = svg.selectAll('.dinerDataPoint')
...
// FIXME - high: null data gaps https://bocoup.com/blog/showing-missing-data-in-line-charts
	...
	// let area = svg.selectAll('.covidDataArea')
	//   	.data([dates], function(d) {
	//   		return d;
	//   	});
...
// ====== Selector ======
var selectorDiv = d3.select('#main')
